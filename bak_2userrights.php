<?php
	...
	$tpl->set_var("ShiftMovementReport","");
	...
	if(isset($_SESSION["usertype"]) && trim($_SESSION["usertype"]) == "employee")
	{
		...
		if($_SESSION["designation"] == "6" || $_SESSION["designation"] == "18" || $_SESSION["designation"] == "19" || $_SESSION["designation"] == "7" || $_SESSION["designation"] == "13")
		{
		...
		if(in_array("AttendanceReport",$_SESSION["userrights"]) || in_array("LeaveReport",$_SESSION["userrights"]) || in_array("BreakExceedReport",$_SESSION["userrights"]) || in_array('LateComingReport',$_SESSION["userrights"]) || in_array('BreakExceedApprovedReport',$_SESSION["userrights"]))
		...
		if(in_array("IssueCategory",$_SESSION["userrights"]) || in_array("PendingTicket",$_SESSION["userrights"]))
			$tpl->parse("ITTicketingBlock",false);

		//print_r($_SESSION["userrights"]);
		//echo $PageIdentifier;

		if(in_array("RctSource",$_SESSION["userrights"]) || in_array("RctDivision",$_SESSION["userrights"]) || in_array("EmployeeTest",$_SESSION["userrights"]) || in_array("HrRound",$_SESSION["userrights"]) || in_array("IQRound",$_SESSION["userrights"]) || in_array("CurrentOpenings",$_SESSION["userrights"]) || in_array("TestRound",$_SESSION["userrights"]) || in_array("FinalHr",$_SESSION["userrights"]) || in_array("Candidates",$_SESSION["userrights"]) || in_array("DocumentDetails",$_SESSION["userrights"]) || in_array("Proof",$_SESSION["userrights"]) || in_array("Graduation",$_SESSION["userrights"]) || in_array("PostGraduation",$_SESSION["userrights"]) || in_array("CandidateDocumentDetails",$_SESSION["userrights"]) || in_array("Joinees",$_SESSION["userrights"]))
		$tpl->parse("DisplayHRBlock",false);


		if(in_array("RctSheet",$_SESSION["userrights"]) || in_array("DocumentReport",$_SESSION["userrights"]) || in_array("PendingDocumentReport",$_SESSION["userrights"]) || in_array("MasterReport",$_SESSION["userrights"]) || in_array("BirthdayCalendar",$_SESSION["userrights"]) || in_array("InductionReport",$_SESSION["userrights"]))
		$tpl->parse("DisplayHRReports",false);

		if($PageIdentifier != "Dashboard" && $PageIdentifier != "Employee Profile" && !in_array($PageIdentifier, $_SESSION["userrights"]))
		{
		...
		if(in_array('ShiftMovement',$_SESSION["userrights"]) || in_array('ShiftMovementRequest',$_SESSION["userrights"]) || in_array('EmergencyShiftMovement',$_SESSION["userrights"]))
		{
	...
	else if(isset($_SESSION["usertype"]) && trim($_SESSION["usertype"]) == "admin")
	{
		...
		$tpl->parse("ShiftMovementReport",false);
		...
		{
			$adm = array("InventoryMake", "InventoryAttributes", "InventoryType", "InventoryLocation", "InventoryVendor", "StockRegister", "InventoryRepairReport", "InventoryPurchaseReport", "StockSummaryReport", "LocationWiseInventoryReport", "IssueCategory", "ITIssues","IssueAccess","TicketCallHistory","RequisitionInventory","ITRequisition","BreakExceedApprovedReport","UserWiseIssueTrackingReport","IssueCategoryWiseIssueTrackingReport","LocationWiseIssueTrackingReport","DateWiseTicketGraph","IssueCategoryWiseTicketGraph","CallStatusWiseMonthlyReport","CurrentInventoryExport");
			...
			$tpl->parse("ITSupportTimingsModule",false);
