<?php
	...
	//echo $sSQL = "select *, date_format(date,'%Y-%m-%d') as date from pms_attendance"; die;
	$sSQL = "SELECT * , date_format( date, '%Y-%m-%d' ) AS date FROM pms_attendance WHERE date_format( `date` , '%Y-%m-%d' ) BETWEEN '2013-03-01' AND '2013-03-31'";
	$db->query($sSQL);
	if($db->num_rows())
	{
		while($db->next_record())
		{
			...
			$sSQL = "select * from pms_attendance1 where user_id='".$db->f("user_id")."' and date_format(date,'%Y-%m-%d') = '".$db->f("date")."'";
			$db1->query($sSQL);
			if($db1->num_rows())
			{
				if($db1->next_record())
				{
						...
						$curRow["shift_id"] = $db1->f("shift_id");
						//echo '<pre>'; print_r($curRow); 
