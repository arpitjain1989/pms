<?php 
	...
	if(isset($_POST["action"]) && trim($_POST["action"]) == "AttendanceSearch")
	{
		...
		$_SESSION["SearchLeave"]["month"] = $_POST["month1"];
		$_SESSION["SearchLeave"]["year"] = $_POST["year1"];
	...
	if(!isset($_SESSION["SearchLeave"]["year"]))
		 $_SESSION["SearchLeave"]["year"] = $curYear;
	...
	if(isset($_SESSION["SearchLeave"]["year"]))
	{
		//echo $_SESSION["SearchLeave"]["year"];
		 $tpl->set_var("year1", $_SESSION["SearchLeave"]["year"]);
		//exit;
	}
	else
		$_SESSION["SearchLeave"]["year"] = 0;
	
	if(isset($_SESSION["SearchLeave"]["month"]))
		$tpl->set_var("month1", $_SESSION["SearchLeave"]["month"]);
		else
		$tpl->set_var("month1", "01");
	...
	if(count($arrEmployee) > 0)
	{
		foreach($arrEmployee as $curEmp)
		{
			...
			/* ----check if monthly report already generated or not-------------------*/
			
			if($curEmp['opening_leave_balance_month'] > $_SESSION["SearchLeave"]["month"]  )
			{
			//exit;
					$openingLeaveBalance = $objLeave->fnGetclosingLeaveBalanceByMonth($curEmp['id'],$_SESSION["SearchLeave"]["month"],$_SESSION["SearchLeave"]["year"]);
					$month = $_SESSION["SearchLeave"]["month"];
					$year = $_SESSION["SearchLeave"]["year"];
					 $date = $_SESSION["SearchLeave"]["year"]."-".$_SESSION["SearchLeave"]["month"]."-01";
					 $first_day_this_month1 = date('Y-m-01', strtotime($date));
	 				 
					 $last_day_this_month1 = date('Y-m-t',strtotime($date));
					 $arrGetAllTotalLeaveForLeaveReport = $objAttendance->fnGetAllLeaveForLeaveReport($curEmp['id'],$first_day_this_month1,$last_day_this_month1);
					//exit;
	
			}
			else if($curEmp['opening_leave_balance_month'] < $_SESSION["SearchLeave"]["month"])
			{
			
			$openingLeaveBalance = $objLeave->fnGetclosingLeaveBalanceByMonth($curEmp['id'],$curEmp['opening_leave_balance_month'],$_SESSION["SearchLeave"]["year"],$status);
