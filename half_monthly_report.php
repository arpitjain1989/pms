<?php
	...
	foreach($employeeList as $employee)
	{
		...
		/* Total break exists */
		$break_exceed_days = $objCalculation->fnGetHalfTotalBreaks($userid,$month,$year,$employee['designation']);

		/* Total late comings */
		$total_late_coming = $objCalculation->fnGetHalfTotalLateComings($userid,$month,$year,$employee['designation']);
//die;
		/* Total presents */
		$in_total = $objCalculation->fnGetHalfTotalPresents($userid,$month,$year,$employee['designation']);
		...
		$MovementDetails = $objCalculation->fnGetHalfTotalOfficialShiftMovementDays($userid,$month,$year);
...
/* for now we not consider shift movement apporval done or not */
		...
		//$total_leave_taken = $total_upl + ($total_uhl * .5 ) + $total_ppl_consumed + $abs + ($ha * .5);
		$total_leave_taken = $total_upl + ($total_uhl * .5 ) + $total_ppl + $abs + ($ha * .5);
...
//die;
		...
		{
			$getAllLeaveRecord = $objCalculation->fnGetAllLeaveRecord($checkRecordExistence,$month,$year);

			//echo 'hello'; print_r($getAllLeaveRecord); die;
			$total_avail_leave = $getAllLeaveRecord['opening'];
			$ClosingLeavesBalance = ($total_avail_leave - $total_ppl_consumed) + $leaves_earn;
			...
			$summary = array("emp_id"=>$userid,"added_no_of_leaves"=>$leaves_earn,"date"=>$current_date,"reason"=>json_encode($reason),"opening_leave"=>$total_avail_leave,"closing_leave"=>$ClosingLeavesBalance,"ishalfmonthly"=>'1');
		...
		$finalCalcHalfMonthly = array("employee_id"=>$userid,"month"=>$month,"year"=>$year,"present"=>$in_total,"break_exceeds"=>$break_exceed_days,"total_plt"=>$total_late_coming,"ppl"=>$total_planned_leave_taken,"uhl"=>$total_uhl,"wo"=>$wo,"ph"=>$total_ph,"ha"=>$ha,"hlwp"=>$hlwp,"abs"=>$abs,"plwp"=>$plwp,"ulwp"=>$ulwp,"nj"=>'0',"le"=>'0',"awol"=>'0',"total_present"=>$total_present,"pay_days"=>$payDays,"opening_leave"=>$total_avail_leave,"leave_earns"=>$leaves_earn,"pl_taken"=>$total_ppl_consumed,"eml_taken"=>'0',"phl_taken"=>$total_phl,"uhl_taken"=>$total_uhl,"total_leave_taken"=>$total_leave_taken,"closing_balance"=>$ClosingLeavesBalance,"upl"=>$total_upl);
		//echo '<pre>'; print_r($finalCalcHalfMonthly);
		$insertHalfReport = $objCalculation -> fnInsertHalfMonthReport($finalCalcHalfMonthly);
		echo "<br/><hr>";
		//die;
	}
