<?php
	...
	$tpl->load_file('roster.html','main_container');
	...
	{
		header("Location: roster_list.php?info=err");
		exit;
	}
...
//print_r($rosterDetail);
						...
						//{
							...
							/* do not change from the roster as changes in leave type already made when the roster is added */
		...
		header("Location: roster_list.php?info=added");
		exit;
	...
	if(count($arrEmployees) > 0)
	{
		//$EmployeeShifts = $objEmployee->fnGetEmployeesShiftByReportingHead($_SESSION["id"]);

		$arrShifts = $objShifts->fnAllowedShiftsDetailsByHeadId($_SESSION["id"]);
		...
		foreach($arrEmployees as $employeeId)
		{
			...
			if(count($arrRosterDates) > 0)
			{
				...
				foreach($arrRosterDates as $date => $displayDate)
				{
					...
					/* Fetch roster information - for update mode */
					...
					/* Default weekoff */
					$setWeekOff = 6;
					if(isset($arrRoster["weekoffday"]) && in_array($arrRoster["weekoffday"], $arrWeekDays))
						$setWeekOff = array_search($arrRoster["weekoffday"], $arrWeekDays);

					$tpl->set_var("setWeekOff",$setWeekOff);
