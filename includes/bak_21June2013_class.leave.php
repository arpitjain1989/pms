<?php
	...
	class leave extends DB_Sql
	{
		...
		function fnInsertHalfLeaveForm($des_id,$arrLeaveForm)
		{
			...
			//echo '<br>hello'.$checkDeligateManagerId .'<br>';  print_r($arrLeaveForm);die;
			//echo 'hello'; die;
			if($shiftCheck != '')
			{
				return 0;
		...
		{
			$arrLeaveFormValues = array();
			$query = "SELECT employee.*,leaves.*,head.name as headname,leaves.id as leaves_id,DATE_FORMAT(leaves.start_date,'%d-%m-%Y') AS start_date FROM `pms_half_leave_form` AS leaves INNER JOIN `pms_employee` AS employee ON leaves.employee_id = employee.id LEFT JOIN `pms_employee` AS head ON employee.teamleader = head.id WHERE employee_id IN($ids) and employee.status = '0' and leaves.isactive = '0'";
				$this->query($query);
				...
				if($_SESSION['designation'] == '6' || $_SESSION['designation'] == '18' || $_SESSION['designation'] == '19')
				{
					...
					else if($post['actual_Designation'] == '5' || $post['actual_Designation'] == '9' || $post['actual_Designation'] == '10'  || $post['actual_Designation'] == '14' || $post['actual_Designation'] == '15' || $post['actual_Designation'] == '16' )
					{
						...
						if($_SESSION['id'] == $post['session_delegateManager'])
						{
							...
							$content = "Dear ".$LeaveInfo["name"].",<br><br>";
							$content .= $DeligatedManagerInfo["name"]." has ".$status[$post["manager_delegate_status"]]." your Leave request on ".$date." from ".$LeaveInfo["startdate"]." to ".$LeaveInfo["enddate"].".<br/><br/>";
							$content .= "<br><br>Regards,<br>".SITEADMINISTRATOR;
...
//print_r($post);
							...
							$content = "Dear ".$LeaveInfo["name"].",<br><br>";
							$content .= $ManagerInfo["name"]." has ".$status[$post["status_manager"]]." your Leave request on ".$date." from ".$LeaveInfo["startdate"]." to ".$LeaveInfo["enddate"].".<br/><br/>";
							$content .= "<br><br>Regards,<br>".SITEADMINISTRATOR;
							//echo 'helo2'.$content.'<br>';
							sendmail($MailTo, $Subject, $content);
					...
					else if($post['actual_Designation'] == '20' || $post['actual_Designation'] == '21' || $post['actual_Designation'] == '22' || $post['actual_Designation'] == '23' || $post['actual_Designation'] == '24' || $post['actual_Designation'] == '25' || $post['actual_Designation'] == '26' || $post['actual_Designation'] == '27' || $post['actual_Designation'] == '28' || $post['actual_Designation'] == '11' || $post['actual_Designation'] == '12' )
					{
						...
						if($_SESSION['id'] == $post['session_delegateManager'])
						{
							...
							$content = "Dear ".$LeaveInfo["name"].",<br><br>";
							$content .= $DeligatedManagerInfo["name"]." has ".$status[$post["manager_delegate_status"]]." your Leave request on ".$date." from ".$LeaveInfo["startdate"]." to ".$LeaveInfo["enddate"].".";
							if($status[$post["manager_delegate_status"]] == "Approved")
							{
						...
						else
						{
							...
							$content = "Dear ".$LeaveInfo["name"].",<br><br>";
							$content .= $ManagerInfo["name"]." has ".$status[$post["status_manager"]]." your Leave request on ".$date." from ".$LeaveInfo["startdate"]." to ".$LeaveInfo["enddate"].".";
							if($status[$post["status_manager"]] == "Approved")
							{
								$content .= " As approved, you can avail these leaves from ".$LeaveInfo["startdate"]." to ".$LeaveInfo["enddate"].".<br/><br/>";
							}
				...
				else if($_SESSION['designation'] == '17')
				{
					...
					{
						$content .= "Your application for Leave/s has been ".$status[$post["status_manager"]]." by ".$ceoInfo["name"]." on ".$LeaveInfo["leavedate"].". As ".$status[$post["status_manager"]].", you can avail these leaves from ".$LeaveInfo["startdate"]." to ".$LeaveInfo["enddate"].".";
						if(isset($delegatedManageDetails["name"]) && trim($delegatedManageDetails["name"]) != "")
							$content .= "<br/><br/>Your responsibilities would be delegated to ".$delegatedManageDetails['name']." while you are on leave/s, which please note.<br/><br/>";
					}
					else
					{
					...
					if($post['status_manager'] == 1)
					{
						if($post['manager_delegate'] != '0')
						{
							if(isset($delegatedManageDetails["email"]) && trim($delegatedManageDetails["email"]) !="")
							{
...
//print_r($post); die;
				...
				if($_SESSION['designation'] == '6' || $_SESSION['designation'] == '18' || $_SESSION['designation'] == '19' || $_SESSION['designation'] == '17')
				{
		...
		function fnUpdateHalfLeaveStatus($post)
		{
			...
			if($_SESSION["usertype"] == "employee")
			{
				...
				if($LeaveInfo['deligateTeamLeaderId'] != '' && $LeaveInfo['deligateTeamLeaderId'] != '0')
				{
					$delegateTeamleaderDetails = $objEmployee->fnGetEmployeeById($LeaveInfo["deligateTeamLeaderId"]);
				}
				if($LeaveInfo['deligateManagerId'] != '' && $LeaveInfo['deligateManagerId'] != '0')
				{
					$delegateManagerDetails = $objEmployee->fnGetEmployeeById($LeaveInfo["deligateManagerId"]);
				}
				...
				if($_SESSION['designation'] == '6' || $_SESSION['designation'] == '18' || $_SESSION['designation'] == '19')
				{
					...
					if($_SESSION['id'] == $LeaveInfo['deligateManagerId'])
					{
						...
						if($LeaveInfo['designation'] == '7' || $LeaveInfo['designation'] == '13')
						{
							/* mail to leave applyer */
							$MailTo = $LeaveInfo["email"];
							$Subject = "Half Leave Request ".$status[$post["manager_delegate_status"]];

							$content = "Dear ".$LeaveInfo["name"].",<br><br>";
							$content .= $delegateManagerDetails["name"]." has ".$status[$post["manager_delegate_status"]]." your half day leave request on ".$date." from ".$LeaveInfo["startdate"]." to ".$LeaveInfo["enddate"].".";
							if($status[$post["manager_delegate_status"]] == 'Approved')
							{
								$content .= "As approved, you can avail these half leave on ".$LeaveInfo["startdate"].".";
							}
							$content .= "<br><br>Regards,<br>".SITEADMINISTRATOR;
							//echo '<br>hello'.$content.'<br>';
							sendmail($MailTo, $Subject, $content);

							/* mail to manager */
							if($LeaveInfo["teamleader"] != '0' && $LeaveInfo["teamleader"] != '0')
							{
								$MailTo = $TeamleaderInfo["email"];
								$Subject = "Half Leave Request ".$status[$post["manager_delegate_status"]];
								$content = "Dear ".$TeamleaderInfo["name"].",<br><br>";
								$content .= $delegateManagerDetails["name"]." has ".$status[$post["manager_delegate_status"]]." half day leave request of ".$LeaveInfo["name"]." on ".$LeaveInfo["leavedate"]." from ".$LeaveInfo["startdate"]." to ".$LeaveInfo["enddate"].".<br/><br/>";
								$content .= "<br><br>Regards,<br>".SITEADMINISTRATOR;
								//echo '<br>hello3'.$content.'<br>';
								sendmail($MailTo, $Subject, $content);
							}
						}
						
						//echo 'hello'; die;
						else if($LeaveInfo["designation"] == '11' || $LeaveInfo["designation"] == '12' || $LeaveInfo["designation"] == '20' || $LeaveInfo["designation"] == '21' || $LeaveInfo["designation"] == '22' || $LeaveInfo["designation"] == '23' || $LeaveInfo["designation"] == '24' || $LeaveInfo["designation"] == '25' || $LeaveInfo["designation"] == '26' || $LeaveInfo["designation"] == '27' || $LeaveInfo["designation"] == '28' )
						{
							//echo 'hello'; die;
							/* mail to leave applyer */
							$MailTo = $LeaveInfo["email"];
							...
							/* mail to actual manager */
							if($LeaveInfo["manager_id"] != '0' && $LeaveInfo["manager_id"] != '0')
							{
								$MailTo = $ManagerInfo["email"];
								$Subject = "Half Leave Request ".$status[$post["manager_delegate_status"]];
								$content = "Dear ".$ManagerInfo["name"].",<br><br>";
						...
						else if($LeaveInfo["designation"] == '5' || $LeaveInfo["designation"] == '9' || $LeaveInfo["designation"] == '10' || $LeaveInfo["designation"] == '14' || $LeaveInfo["designation"] == '15' || $LeaveInfo["designation"] == '16')
						{
							//echo 'hello1'; die;
							/* mail to applyer */
							$MailTo = $LeaveInfo["email"];
							...
							/* mail to delegated teamleader */
							if($LeaveInfo['deligateTeamLeaderId'] != '' && $LeaveInfo['deligateTeamLeaderId'] != '0')
							{
								$MailTo = $delegateTeamleaderDetails["email"];
								$Subject = "Half Leave Request ".$status[$post["manager_delegate_status"]];
								$content = "Dear ".$delegateTeamleaderDetails["name"].",<br><br>";
								$content .= $delegateManagerDetails["name"]." has ".$status[$post["manager_delegate_status"]]." half day leave request of ".$LeaveInfo["name"]." on ".$LeaveInfo["leavedate"]." from ".$LeaveInfo["startdate"]." to ".$LeaveInfo["enddate"].".<br/><br/>";
								$content .= "<br><br>Regards,<br>".SITEADMINISTRATOR;
								//echo '<br>hello3'.$content.'<br>';
								sendmail($MailTo, $Subject, $content);
					...
					else
					{
						...
						if($LeaveInfo['designation'] == '7' || $LeaveInfo['designation'] == '13')
						{
							...
							$content = "Dear ".$LeaveInfo["name"].",<br><br>";
							$content .= $TeamleaderInfo["name"]." has ".$status[$post["status_manager"]]." your half day leave request on ".$date." from ".$LeaveInfo["startdate"]." to ".$LeaveInfo["enddate"].".";
							if($status[$post["status_manager"]] == 'Approved')
							{
								$content .= " As approved, you can avail these half leave on ".$LeaveInfo["startdate"].".";
							}
							...
							{
								/* mail to applyer */
								$MailTo = $delegateManagerDetails["email"];
								$Subject = "Half Leave Request ".$status[$post["status_manager"]];
								$content = "Dear ".$delegateManagerDetails["name"].",<br><br>";
								$content .= $TeamleaderInfo["name"]." has ".$status[$post["status_manager"]]." half day leave request of ".$LeaveInfo["name"]." on ".$LeaveInfo["leavedate"]." from ".$LeaveInfo["startdate"]." to ".$LeaveInfo["enddate"].".<br/><br/>";
								$content .= "<br><br>Regards,<br>".SITEADMINISTRATOR;
								//echo '<br>hello3'.$content.'<br>';
								sendmail($MailTo, $Subject, $content);
							}
						...
						else if($LeaveInfo["designation"] == '11' || $LeaveInfo["designation"] == '12' || $LeaveInfo["designation"] == '20' || $LeaveInfo["designation"] == '21' || $LeaveInfo["designation"] == '22' || $LeaveInfo["designation"] == '23' || $LeaveInfo["designation"] == '24' || $LeaveInfo["designation"] == '25' || $LeaveInfo["designation"] == '26' || $LeaveInfo["designation"] == '27' || $LeaveInfo["designation"] == '28' )
						{
							...
							if($LeaveInfo['deligateManagerId'] != '' && $LeaveInfo['deligateManagerId'] != '0')
							{
								...
								$Subject = "Half Leave Request ".$status[$post["status_manager"]];
								$content = "Dear ".$delegateManagerDetails["name"].",<br><br>";
								$content .= $ManagerInfo["name"]." has ".$status[$post["status_manager"]]." half day leave request of ".$LeaveInfo["name"]." on ".$LeaveInfo["leavedate"]." from ".$LeaveInfo["startdate"]." to ".$LeaveInfo["enddate"].".<br/><br/>";
								$content .= "<br><br>Regards,<br>".SITEADMINISTRATOR;
								//echo '<br>hello3'.$content.'<br>';
								sendmail($MailTo, $Subject, $content);
							}
						...
						else if($LeaveInfo["designation"] == '5' || $LeaveInfo["designation"] == '9' || $LeaveInfo["designation"] == '10' || $LeaveInfo["designation"] == '14' || $LeaveInfo["designation"] == '15' || $LeaveInfo["designation"] == '16')
						{
							...
							$content = "Dear ".$LeaveInfo["name"].",<br><br>";
							$content .= $ManagerInfo["name"]." has ".$status[$post["status_manager"]]." your half day leave request on ".$date." from ".$LeaveInfo["startdate"]." to ".$LeaveInfo["enddate"].".";
							if($status[$post["status_manager"]] == 'Approved')
							{
								$content .= "As approved, you can avail these half leave on ".$LeaveInfo["startdate"].".";
							}
...
//echo 'hell2';
				...
				{
					//echo 'h12';
					/* check login teamleader is actual of delegated */

					if($_SESSION['id'] == $LeaveInfo['deligateTeamLeaderId'])
					{	
						
						/* This is deleagated teamleader */
						...
						$content = "Dear ".$TeamleaderInfo["name"].",<br><br>";
						$content .= $delegateTeamleaderDetails["name"]." has ".$status[$post["delegate_status"]]." half day leave request of ".$LeaveInfo["name"]." on ".$LeaveInfo["leavedate"]." from ".$LeaveInfo["startdate"]." to ".$LeaveInfo["enddate"].".";
						$content .= "<br><br>Regards,<br>".SITEADMINISTRATOR;
						//echo '<br>hello2'.$content.'<br>';
						sendmail($MailTo, $Subject, $content);
						...
						$content = "Dear ".$LeaveInfo["name"].",<br><br>";
						$content .= $delegateTeamleaderDetails["name"]." has ".$status[$post["delegate_status"]]." your half day Leave request on ".$date." from ".$LeaveInfo["startdate"]." to ".$LeaveInfo["enddate"].".";
						if($status[$post["delegate_status"]] == 'Approved')
						{
							$content .= " As approved, you can avail these half leave on ".$LeaveInfo["enddate"].".";
						...
						if($LeaveInfo["manager_id"] != '0' && $LeaveInfo["manager_id"] != '0')
						{
							...
							$Subject = "Half Leave Request ".$status[$post["delegate_status"]];
							$content = "Dear ".$ManagerInfo["name"].",<br><br>";
							$content .= $delegateTeamleaderDetails["name"]." has ".$status[$post["delegate_status"]]." half day leave request of ".$LeaveInfo["name"]." on ".$LeaveInfo["leavedate"]." from ".$LeaveInfo["startdate"]." to ".$LeaveInfo["enddate"].".<br/><br/>";
							$content .= "<br><br>Regards,<br>".SITEADMINISTRATOR;
							//echo '<br>hello3'.$content.'<br>';
							sendmail($MailTo, $Subject, $content);
						}

						/* send mail to delegate manager */
						if($LeaveInfo['deligateManagerId'] != '' && $LeaveInfo['deligateManagerId'] != '0')
						{
							//$delegateManagerDetails = $objEmployee->fnGetEmployeeById($LeaveInfo["deligateManagerId"]);
							$MailTo = $delegateManagerDetails["email"];
					...
					else
					{
						...
						$content = "Dear ".$LeaveInfo["name"].",<br><br>";
						$content .= $TeamleaderInfo["name"]." has ".$status[$post["status"]]." your half day Leave request on ".$date." from ".$LeaveInfo["startdate"]." to ".$LeaveInfo["enddate"].".";
						if($status[$post["status"]] == 'Approved')
						{
							$content .= " As approved, you can avail these half leave on ".$LeaveInfo["enddate"].".";
						...
						$content = "Dear ".$delegateTeamleaderDetails["name"].",<br><br>";
						$content .= $TeamleaderInfo["name"]." has ".$status[$post["status"]]." half day leave request of ".$LeaveInfo["name"]." on ".$LeaveInfo["leavedate"]." from ".$LeaveInfo["startdate"]." to ".$LeaveInfo["enddate"].".<br/><br/>";
						$content .= "<br><br>Regards,<br>".SITEADMINISTRATOR;
						//echo '<br>hello2'.$content.'<br>';
						sendmail($MailTo, $Subject, $content);
				...
				else
				{
					...
					/* mail to delegate teamleader */
					//if($LeaveInfo['deligateTeamLeaderId'] != '' && $LeaveInfo['deligateTeamLeaderId'] != '0')
					//{
						/* send mail to delegate teamleader */
					//}
					...
					$content = "Dear ".$LeaveInfo["name"].",<br><br>";
					$content .= $TeamleaderInfo["name"]." has ".$status[$post["status"]]." your half day Leave request on ".$LeaveInfo["leavedate"]." from ".$LeaveInfo["startdate"]." to ".$LeaveInfo["enddate"].".<br/><br/>";
					$content .= "<br><br>Regards,<br>".SITEADMINISTRATOR;
					//echo '<br>hello2'.$content.'<br>';
					sendmail($MailTo, $Subject, $content);
