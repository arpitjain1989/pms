...
include_once('db_mysql.php');
	class calculation extends DB_Sql
	{
		...
		function fnGetTotalBreacks($id,$month,$year,$des)
		{
		...
		function fnGetHalfTotalLateComings($id,$month,$year,$des)
		{
			...
			if($des == '7' || $des == '13')
			{
				$query = "SELECT COUNT(is_late) as late_comings FROM `pms_attendance` WHERE DATE_FORMAT(date,'%Y-%m-%d') between '$start_date' AND '$end_date' AND user_id = '$id' AND `is_late` = 1 AND `ishoursapproved`=0 and `total_working_hours` < '07:20:00'";
			}
			else
			{
				$query = "SELECT COUNT(is_late) as late_comings FROM `pms_attendance` WHERE DATE_FORMAT(date,'%Y-%m-%d') between '$start_date' AND '$end_date' AND user_id = '$id' AND `is_late` = 1 AND `ishoursapproved`=0";
			}
		...
		function fnGetHalfTotalPresents($id,$month,$year)
		{
			...
			$query = "SELECT at.*,COUNT( id ) AS total_in_time_count, `total_working_hours`  as diff FROM `pms_attendance` as at WHERE DATE_FORMAT(at.date,'%Y-%m-%d') between '$start_date' AND '$end_date' AND at.user_id = '$id' and (`total_working_hours`	 > '06:00:00')";
			$this->query($query);
		...
		function fnGetHalfTotalLeave($id,$month,$year,$leaveType)
		{
			...
			echo $query = "SELECT COUNT(leave_id) as leaves FROM `pms_attendance` WHERE DATE_FORMAT(date,'%Y-%m-%d') between '$start_date' AND '$end_date' AND user_id = '$id' AND `leave_id` = '$leave_id'";
			//$query = "SELECT * FROM `pms_attendance` WHERE ";
			$this->query($query);
