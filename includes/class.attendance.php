<?php
	...
	class attendance extends DB_Sql
	{
		...
		function fnInsertAttendance($arrEmployee)
		{
			...
			foreach ($arrEmployee['hdnemployeeid'] as $value)
			{
				...
				*/
					...
					//echo $query;
						...
						if($this->num_rows() > 0)
						{
							if($this->next_record())
							{
								...
								$diffTime = strtotime($differ1);
								$actualtime = strtotime('06:40:00');
								
								if($diffTime < $actualtime )
								{
									 $text = 'Short Fall of Work Hours';
								}
								else 
								{
									 $text = '';
				...
				if(isset($arrEmployee['enche_'.$value]))
				{
				 $enche = $arrEmployee['enche_'.$value];
				}
				else
				{
				$enche = 0;
				...
				$this->fnCalculateHalfMonthlyReport1($arrNewRecords["user_id"],$curDay,$curMonth,$curYear,$emp_designation, $objCalculationTime);
				//die;
		...
		function fnAutoLeaveStatusUpdation($userId, $curDate, $curMonth, $curYear)
		{
			...
			{
				if($db->next_record())
				if($db->f("opening_leave_balance_month")== $curMonth)
					{
					$checkFromDate = $db->f("opening_leave_balance_year")."-".$db->f(			"opening_leave_balance_month")."-01";
					}
					else if($curMonth<$db->f("opening_leave_balance_month"))
					{
						$checkFromDate = $curYear."-".$curMonth."-01";
					}
			}
			...
			$arrLwp = array();
			/* changes for  lwp_from and lwp_to */
			 $sSQL = "select date_format(lwp_date,'%Y-%m-%d') as lwp_dt,date_format(lwp_date_to,'%Y-%m-%d') as lwp_dt_to from pms_approved_lwp where user_id='".mysql_real_escape_string($userId)."' and date_format(lwp_date,'%Y-%m-%d') >= '".$checkFromDate."'and date_format(lwp_date_to,'%Y-%m-%d') <= '".date('Y-m-t', strtotime($checkFromDate))."' and approval_status='1'";
			$db->query($sSQL);
			if($db->num_rows() > 0)
			{
				while($db->next_record())
				{
					$startDate = $db->f("lwp_dt");
					$endDate = 	$db->f("lwp_dt_to");
					if(!empty($endDate))
					{
...
//echo "<br>";
			...
			/* Fetch rostered WO */
			$sSQL = "select date_format(weekoffdate,'%Y-%m-%d') as date from pms_roster r where date_format(weekoffdate,'%Y-%m') = '".$curYear."-".$curMonth."' and userid='".$userId."'";
			$this->query($sSQL);
			if($this->num_rows() > 0)
			{
				while($this->next_record())
				{
					$weekOfDays[] = $this->fetchRow();
				}
			}
			...
			$phDays = $objCalculation->fnGetPhs1($userId,$curMonth,$curYear,$EmployeeInfo['rel_date_by_manager'],$EmployeeInfo['d_of_join']);
			if(count($phDays) > 0)
			{
				...
				if(in_array($curDateArr["leave_id"],array(1,2,3,6,7,17)))
				{
					if($curDateArr["isWoPh"] == 1)
					{
						...
						{					
							...
							//echo $userId."pend".$pendingLeaves."";
							if($pendingLeaves >= 1)
							{
							...
							else
							{
								if($pendingLeaves > 0)
								{
									...
									else
									{
										...
										if($mb->num_rows() > 0)
										{
											$sSQL = "update pms_attendance set leave_id = '".$arrInfo['leave_id']."',text = 'No leave balance and no lwp for sandwich', last_modified = '".Date('Y-m-d H:i:s')."' where user_id='".$arrInfo['user_id']."' and date_format(date,'%Y-%m-%d') = '".$arrInfo['date']."'";
											$mb->query($sSQL);
										}
									}
								}
								else
								{
									...
									if($mb->num_rows() > 0)
									{
										$sSQL = "update pms_attendance set leave_id = '".$arrInfo['leave_id']."',text = 'In Out time not Added Or Sandwich', last_modified = '".Date('Y-m-d H:i:s')."' where user_id='".$arrInfo['user_id']."' and date_format(date,'%Y-%m-%d') = '".$arrInfo['date']."'";
										$mb->query($sSQL);
									}
								}
							}
...
*/	
								...
								while($start_date <= $end_date && in_array($curDateArr["leave_id"],array(1,2,3,6,7,17)))
								{
									if(in_array($start_date,$arrPhAndWo) && !in_array($start_date, $arrSandwitchPhAndWo))
									{
										...
										$next_date = date('Y-m-d', strtotime('+1 day', strtotime($start_date)));
						
										$sSQL = "select * from pms_attendance where user_id='".mysql_real_escape_string($userId)."' and date_format(date,'%Y-%m-%d') = '".mysql_real_escape_string($prev_date)."' OR date_format(date,'%Y-%m-%d') = '".mysql_real_escape_string($next_date)."' AND in_time !='00:00:00' and out_time !='00:00:00'";
										
										$mb->query($sSQL);
										if($mb->num_rows() > 0)
										{
											$leaveId = $objLeave->fnGetLeaveTypeIdByTitle('WO');
											$sSQL = "update pms_attendance set leave_id = '".$leaveId."', last_modified = '".Date('Y-m-d H:i:s')."' where user_id='".$userId."' and date_format(date,'%Y-%m-%d') = '".$start_date."'";
												...
												else
												{
													if($pendingLeaves > 0 || ($isContinuous == true && $pendingLeaves == 0))
													{
														...
														else
														{
															...
															if($mb->num_rows() > 0)
															{
																$sSQL = "update pms_attendance set leave_id = '".$arrInfo['leave_id']."',text = 'No leave balance and no lwp ', last_modified = '".Date('Y-m-d H:i:s')."' where user_id='".$arrInfo['user_id']."' and date_format(date,'%Y-%m-%d') = '".$arrInfo['date']."'";
																$mb->query($sSQL);
															}
														}
													}
													else
													{
														...
														if($mb->num_rows() > 0)
														{
															$sSQL = "update pms_attendance set leave_id = '".$arrInfo['leave_id']."',text = 'In Out time not Added', last_modified = '".Date('Y-m-d H:i:s')."' where user_id='".$arrInfo['user_id']."' and date_format(date,'%Y-%m-%d') = '".$arrInfo['date']."'";
															$mb->query($sSQL);
														}
													}
												}
						...
						else
						{
							if(!in_array($curDate,$arrLwp))
							{
								...
								if($mb->num_rows() > 0)
								{
									$sSQL = "update pms_attendance set leave_id = '".$arrInfo['leave_id']."',text = 'No In Out Time Added', last_modified = '".Date('Y-m-d H:i:s')."' where user_id='".$userId."' and date_format(date,'%Y-%m-%d') = '".$curDate."'";
									$mb->query($sSQL);
								}
							}
				...
				else if(in_array($curDateArr["leave_id"],array(4,5,8,12)))
				{
					...
					else
					{
						...
						if($mb->num_rows() > 0)
						{
							$sSQL = "update pms_attendance set leave_id = '".$arrInfo['leave_id']."',text = 'Shortfall of work hours', last_modified = '".Date('Y-m-d H:i:s')."' where user_id='".$userId."' and date_format(date,'%Y-%m-%d') = '".$curDate."'";
							$mb->query($sSQL);
						}
		...
		function fnSendAbsentMail($user)
		{
			...
			if($this->num_rows())
			{
				if($this->next_record())
				{
					...
					/* Check if approved leave for the date is added */
					$sSQL = "SELECT date_format(start_date,'%Y-%m-%d') as start_date , date_format(end_date,'%Y-%m-%d') as end_date, date_format(approved_date_manager, '%Y-%m-%d') as approved_date_manager, date_format(manager_delegate_date, '%Y-%m-%d') as manager_delegate_date, status_manager, manager_delegate_status, isemergency FROM pms_leave_form WHERE '".mysql_real_escape_string($lastDt)."' BETWEEN DATE_FORMAT(start_date,'%Y-%m-%d') AND DATE_FORMAT(end_date,'%Y-%m-%d') and employee_id = '".mysql_real_escape_string($user)."' and (status_manager ='1' or (status_manager ='0' and deligateManagerId!='0' and manager_delegate_status ='1')) order by id desc";
					$db->query($sSQL);
					if($db->num_rows() == 0)
					{
						$sSQL = "select *, date_format(date,'%Y-%m-%d') as adate from pms_attendance where user_id='".mysql_real_escape_string($user)."' and (date_format(date,'%Y-%m-%d') =  date_format(DATE_SUB('".$lastDt."',INTERVAL 1 DAY),'%Y-%m-%d') or date_format(date,'%Y-%m-%d') =  date_format(DATE_SUB('".$lastDt."',INTERVAL 2 DAY),'%Y-%m-%d')) and leave_id='3'";
						$this->query($sSQL);
						if($this->num_rows() == 2)
						{
							$flag = true;
							
							/* Check if leave is already taken */
							while($this->next_record())
							{
								$a_date = $this->f("adate");
								
								$sSQL = "SELECT date_format(start_date,'%Y-%m-%d') as start_date , date_format(end_date,'%Y-%m-%d') as end_date, date_format(approved_date_manager, '%Y-%m-%d') as approved_date_manager, date_format(manager_delegate_date, '%Y-%m-%d') as manager_delegate_date, status_manager, manager_delegate_status, isemergency FROM pms_leave_form WHERE '".mysql_real_escape_string($a_date)."' BETWEEN DATE_FORMAT(start_date,'%Y-%m-%d') AND DATE_FORMAT(end_date,'%Y-%m-%d') and employee_id = '".mysql_real_escape_string($user)."' and (status_manager ='1' or (status_manager ='0' and deligateManagerId!='0' and manager_delegate_status ='1')) order by id desc";
								$db->query($sSQL);
								if($db->num_rows() > 0)
								{
									$flag = false;
...
//die;
		...
		function fnSaveCompensation($compensationInfo)
		{
			...
			if($compensationInfo["delegatedtl_id"] != 0)
			{
				...
				if(count($DelegatedTL) > 0)
				{
					...
					$content = "Dear ".$DelegatedTL['name'].", <br /><br />".$employeeInfo["name"]." has added a compensation for late comming on ".$compensationInfo["exceedondate"]."<br /><br />";
		...
		function fnUserCompensationById($CompensationId)
		{
			...
			if($this->num_rows())
			{
				if($this->next_record())
				{
					...
					$tempCompensation["approvedby_tl"] = $approvedby_tl;
					$tempCompensation["approvedby_delegatedtl"] = $approvedby_delegatedtl;
		...
		function fnGetLateComingsByYearAndMonth($year, $month, $viewall)
		{
			...
			if($viewall == "0")
			{
				...
				$cond = "  and e.designation in (".$desIds.")";
				
				$havingCond = " having count(a.id) > 3";
			}
			...
			$sSQL = "select a.user_id, count(a.id) as totallate, e.name, e1.name as teamleader from pms_attendance a LEFT JOIN pms_employee e ON e.id = a.user_id  LEFT JOIN pms_employee e1 ON e.teamleader = e1.id where a.is_late='1' and late_time > '00:04:00' and date_format(a.date,'%Y')='".mysql_real_escape_string($year)."' and date_format(a.date,'%m')='".mysql_real_escape_string($month)."' ".$cond." group by a.user_id ".$havingCond;
			$this->query($sSQL);
			if($this->num_rows() > 0)
			{
				while($this->next_record())
				{
					...
					$arrLateCommingInformation[$this->f("user_id")]["teamleader"] = $this->f("teamleader");
		...
		function fnCalculateHalfMonthlyReport1($userid,$day,$month,$year,$emp_des, $calculationinfo = array())
		{
			include_once("class.calculation.php");
			include_once("class.employee.php");
			...
			while($first_date <= $to_date)
			{
				...
				$arrPayDays = $this->fnHalfMonthlyDetailCalculation($userid,$curDay,$curMonth,$curYear,$emp_des, $calculationinfo = array());

				if(isset($arrPayDays["PayDays"]) && $arrPayDays["PayDays"] >= 15)
				{
					/* Paydays more than 15 save the data*/

					/* Check if data already added */
					
					$sSQL = "select id from pms_leave_history where emp_id='".mysql_real_escape_string($userid)."' and month='".mysql_real_escape_string($curMonth)."' and year='".mysql_real_escape_string($curYear)."' and ishalfmonthly='1'";
					$db->query($sSQL);
					...
					if($db->num_rows() > 0)
					{
						if($db->next_record())
						{
							...
							$insertSummary = $objCalculation->fnUpdateHalfSummary($db->f("id"), $arrPayDays["Summary"],$curMonth,$curYear);

							$insertRemainingLeaves = $objCalculation->fnUpdateRemainingLeave($userid,$arrPayDays["ClosingLeavesBalance"]);

							/* save the record in summary log table always*/
							$insertSummary1 = $objCalculation->fnInsertHalfSummaryLog($arrPayDays["Summary"],$curMonth,$curYear);
						}
						else
						{
							/* Insert data into table */
							$insertSummary = $objCalculation->fnInsertHalfSummary($arrPayDays["Summary"],$curMonth,$curYear);
				...
				else
				{
					...
					if($db->num_rows() > 0)
					{
						if($db->next_record())
						{
							$sSQL = "delete from pms_leave_history where id='".mysql_real_escape_string($db->f("id"))."'";
							$db->query($sSQL);
						}
					}
				}
		...
		function fnHalfMonthlyDetailCalculation($userid,$day,$month,$year,$emp_des, $calculationinfo = array())
		{
			include_once("class.calculation.php");
			include_once("class.employee.php");
...
//$ulwp = 3;
	...
	/* for now we not consider shift movement apporval done or not */
			...
			$payDays = $total_present + $total_ppl_consumed;
			...
			$arrReturnValue = array("PayDays"=>$payDays, "ClosingLeavesBalance"=>$ClosingLeavesBalance, "Reason"=>$reason, "Summary"=>$summary);
...
//echo "<br/>userid: ".$userid;
				...
				if((($attendance_month == $actual_cur_month) && ($day == $actual_cur_month_last_date)) ||  ($attendance_month == $prev_month))
				{
					...
					/* Total Enched Ph*/
			$total_enche = $objCalculation->fnGetTotalEnche($userid,$month,$year,"P");
					...
					{
						$RemainingPH = $GetTotalPh - $total_ph - $total_enche;
					}
					else
					{
						$RemainingPH = '0';
					...
					//echo "EligiblePhForMonth=".$EligiblePhForMonth."<br>";
					...
					{
						$carryForwardedPh = $EligiblePhForMonth - $total_ph - $total_enche;
					}
					else
					{
						$carryForwardedPh = '0';
					}
					//echo "carryForwardedPh= " .$carryForwardedPh."<br>";
			...
			/* for now we not consider shift movement apporval done or not */
					...
					$total_present = $in_total + $final_wo + $total_ph + $total_spl + ($total_uhl * .5) + ($total_phl * .5) + ( $ha * .5 ) + ( $hlwp * .5 ) + $total_enche; ;
					...
					echo $leaves_earn ;
							echo "<br>";
					echo $opening_leave_balance_org."<br>";
		...
		function fnGetPublicHolidays($fromDate, $toDate, $userId)
		{
			...
			$query = "SELECT a.id, date_format(a.date, '%Y-%m-%d') as startdate, h.title as holiday_title FROM `pms_attendance` a LEFT JOIN pms_holidays h ON date_format(h.holidaydate, '%Y-%m-%d') = DATE_FORMAT(a.date,'%Y-%m-%d') WHERE DATE_FORMAT(a.date,'%Y-%m-%d') between '".mysql_real_escape_string($fromDate)."' AND '".mysql_real_escape_string($toDate)."' AND a.user_id = '".mysql_real_escape_string($userId)."' AND a.leave_id ='10' and date_format(a.date, '%Y-%m-%d') in (select date_format(holidaydate, '%Y-%m-%d') as holidaydate from pms_holidays where date_format(holidaydate,'%Y-%m-%d') between '".mysql_real_escape_string($fromDate)."' AND '".mysql_real_escape_string($toDate)."')";
			$this->query($query);
			if($this->num_rows() > 0)
			{
				while($this->next_record())
				{
					$arrAllPublicHolidays[] = array(
											'id' => $this->f("id"),
											'title' => $leave_title . " - " . $this->f("holiday_title"),
											'start' => $this->f("startdate"),
											'color' => $leave_color
										);
				}
			...
			$sSQL = "select id, date_format(holidaydate, '%Y-%m-%d') as holidaydate, title as holiday_title from pms_holidays where date_format(holidaydate,'%Y-%m-%d') between '".mysql_real_escape_string($fromDate)."' AND '".mysql_real_escape_string($toDate)."' $cond";
			$this->query($sSQL);
			if($this->num_rows() > 0)
			{
				while($this->next_record())
		...
		function fnGetLateComingCompensationByYearAndMonth($year,$month)
		{
			 $arrAllweekOffs = array();
			 $query = "SELECT a.*, b.* FROM `pms_attendance` a INNER JOIN  	pms_exceed_compensation b ON b.attendance_id = a.id where date_format(a.date,'%Y')='".mysql_real_escape_string($year)."' and date_format(a.date,'%m')='".mysql_real_escape_string($month)."' and isLateAllowed =0";
