...
//echo '<br>smPltCount'.$smPltCount;
		...
		function fnGetMonthlyReport($month,$year)
		{
			...
			$query = "SELECT e.name, h.*, e1.name as reporting_head FROM  `pms_attendance_report` h INNER JOIN pms_employee e ON h.employee_id = e.id LEFT JOIN pms_employee e1 ON e.teamleader = e1.id WHERE h.month = '$month' AND h.year = '$year' AND (e.status = '0' OR (e.status = '1' and '".$year."-".$month."' <= date_format( e.relieving_date_by_manager, '%Y-%m'))) order by e.name";
...
//echo '<br>smPltCount'.$smPltCount;
		...
		function fnGetPhs($eid,$month,$year)
		{
			$start_date = $year.'-'.$month.'-'.'01';
			$end_date = date("Y-m-t", strtotime($start_date));
			
			$arrAllweekOffs = array();
			$query = "SELECT holidaydate FROM `pms_holidays` WHERE DATE_FORMAT(holidaydate,'%Y-%m') = '".$year."-".$month."'";
			$this->query($query);
		...
		function fnGetTotalPhOfCurrentMonth($id,$month,$year)
		{
			$start_date = $year.'-'.$month.'-'.'01';
			$end_date = date("Y-m-t", strtotime($start_date));
